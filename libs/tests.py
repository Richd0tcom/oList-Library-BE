...
class TestListBook(APITestCase):
    def setUp(self):
        ...
        #UPDATE BOOK
        self.new_bk = Book.objects.create(name="tst_bk", edition=3, publication_year="2005")
        self.new_bk.save()
    ...
    def test_should_create_book(self):
        ...
        response = self.client.post(reverse("books"), data=json.dumps(self.sample_book),content_type='application/json')
        return self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    
    def test_should_update_book(self):
        
        response = self.client.put(reverse("book", kwargs={'pk':self.new_bk.id}), data=json.dumps(self.sample_book2), content_type='application/json')
